				...
				<div class="col-sm-3">
					<div class="form-group">
						...
						<div class="input-group">
							...
							@php
								$transaction_date = !empty($sell->return_parent->transaction_date) ? $sell->return_parent->transaction_date : date("m/d/y H:i");
							
							@endphp
							{!! Form::text('transaction_date', @format_datetime($transaction_date), ['class' => 'form-control', 'readonly', 'required','id'=>'transaction_date']); !!}
						</div>
					</div>
				</div>
				<div class="col-sm-12">
					<table class="table bg-gray" id="sell_return_table">
				        ...
				        <tbody>
				            <?php
				            $returned_line=\DB::table('transaction_sell_lines')->where('transaction_id',$sell->id)->where('variation_id',Request::get('variation_id'))->first();
				            ?>
				            ...
				            <tr>
				              	...
				              	<td>
				              	    @if($returned_line !==NULL && $sell_line->id==$returned_line->id)
				              	    <?php $Q_returned=$sell_line->quantity_returned + 1; ?>
						            <input type="text" name="products[{{$loop->index}}][quantity]" value="{{$Q_returned}}"
