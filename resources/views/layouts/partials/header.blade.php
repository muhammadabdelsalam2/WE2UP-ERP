...
.skin-blue .content-wrapper .content-header-custom a, .skin-blue .content-wrapper .content-header-custom h1, .skin-blue .content-wrapper .content-header-custom small, .skin-blue-light .content-wrapper .content-header-custom a, .skin-blue-light .content-wrapper .content-header-custom h1, .skin-blue-light .content-wrapper .content-header-custom small {
  ...
  <header class="main-header no-print">
    <a href="{{route('home')}}" class="logo" style="background:#40475b">
        ...
        @if(in_array('pos_sale', $enabled_modules))
          @can('sell.create')
            <a href="{{action('SellPosController@create')}}" title="@lang('sale.pos_sale')" data-toggle="tooltip" data-placement="bottom" class="btn btn-success btn-flat pull-left m-8 hidden-xs btn-sm mt-10">
             ...
             @can('pos_lite')
            <div class="dropdown">
  <button class="btn btn-success btn-flat pull-left m-8 hidden-xs btn-sm mt-10 dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
    @lang('sale.pos_sale') Lite
  </button>
  <?php
  ...
  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
      <a class="dropdown-item" href="/pos/create2/0">سعر البيع الافتراضي</a>
     @foreach($price_groups as $row)
        <a class="dropdown-item" href="/pos/create2/{{$row->id}}">{{$row->name}}</a>
    @endforeach
  </div>
</div>
    @endcan 
