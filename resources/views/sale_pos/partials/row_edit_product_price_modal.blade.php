		...
		<div class="modal-body">
			<div class="row">
			    ...
			    <div class="form-group col-xs-12 ">
				    ...
				    <span class="display_currency " >{{@num_format($product->default_purchase_price)}}</span>
				...
				<div class="form-group col-xs-12 @if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
					<label>@lang('sale.unit_price')</label>
						<input type="text" id="unit_price_modal" name="products[{{$row_count}}][unit_price]" class="form-control pos_unit_price input_number mousetrap" value="{{@num_format(!empty($product->unit_price_before_discount) ? $product->unit_price_before_discount : $product->default_sell_price)}}">
			        <span id="al_mes" style="color:red;display:none">  لايمكنك البيع باقل من سعر الشراء</span>
				</div>
			
				 <?php if (!auth()->user()->can('sales.less_than_purchase_price') ) {?>
				
				<script>
				    $("#unit_price_modal").change(function(){
				       var unit_price = $(this).val();
				       var purchase   = {{@num_format($product->default_purchase_price)}};
				       if(unit_price < purchase){
				           $(this).val(purchase);
				           $("#al_mes").show();
				       }
				    });
				...
				<div class="form-group col-xs-12 col-sm-6 @if(!$edit_discount) hide @endif">
					...
					<script>
					    $(".row_discount_amount").change(function(){
					        debugger;
					         $("#al_mes_dis").hide();
					       var type = $(".row_discount_type").val();
					       var max_discount={{$product->max_discount}};
					       var current_val=$(this).val();
					       var unit_price=$("#unit_price_modal").val(); 
					       if(type=='fixed'){
					          var max_fixed_discount=parseFloat(max_discount) * parseFloat(unit_price) / 100;
					              max_discount=parseFloat(max_fixed_discount).toFixed(2);
					            
					       }
					       if(current_val>max_discount){
				...
				<span id="al_mes_dis" style="color:red;display:none">      لقد تجاوزت اقصي قيمه لخصم المنتج </span>
				</div>
