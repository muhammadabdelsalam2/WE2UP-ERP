<div class="row">
		...
		<div class="{{$col_class}}">
			<div class="form-group @if($readonly) hide @endif">
				...
				<div class="input-group">
					...
					{!! Form::select("payment[$row_index][account_id]", $accounts, !empty($payment_line['account_id']) ? $payment_line['account_id'] : '' , ['required'=>'required','class' => 'form-control select2 account-dropdown', 'id' => !$readonly ? "account_$row_index" : "account_advance_$row_index", 'style' => 'width:100%;', 'disabled' => $readonly]); !!}
				</div>
			</div>
		</div>
	@endif
	...
	@if( Request::segment(1)=='pos')
		@include('sale_pos.partials.payment_type_details_pos')
	@else
