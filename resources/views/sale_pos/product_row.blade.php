<tr class="product_row" data-row_index="{{$row_count}}">
	<td>
		...
		@php
			...
			$max_in_invoice = $product->max_in_invoice ?? 100000000;
...
<?php if(auth()->user()->can('sales.show_current_stock_in_pose')){ ?>
    <td>

        <?php echo number_format($product->qty_available, 2, '.', ',');?>
	...
	<td>
		...
		<input type="text" data-min="1" 
			...
			@if($product->enable_stock && empty($pos_settings['allow_overselling']) )
				...
				data-max-invoice-rule="{{$max_in_invoice}}"
			
				data-max-invoice-msg="@lang('validation.custom-messages.quantity_not_available', ['qty'=> $product->max_in_invoice, 'unit' => $product->unit  ])"
		...
		<span class="input-group-btn"><button type="button" class="btn btn-default btn-flat quantity-up"><i class="fa fa-plus text-success"></i></button></span>
		 ...
		 <span class="max_in_invoice_msg"style="color:red;display:none;"> اقصي قيمة للمنتج في الفاتورة هي  {{ $max_in_invoice}}</span>
	</td><td>	
	...
	<td class="text-center v-center">
	    		<input type="hidden" name="products[{{$row_count}}][max_in_invoice]" class="form-control max_in_invoice input_number" value="{{@num_format($max_in_invoice)}}" >
		...
		<span class="display_currency pos_line_purchase_text  hide" style="color:red" data-currency_symbol="true">{{$product->quantity_ordered*$product->default_purchase_price}}</span>
		<input type="hidden" name="products[{{$row_count}}][unit_purchase_price]" class="form-control unit_purchase_price input_number" value="{{@num_format($product->default_purchase_price)}}">
