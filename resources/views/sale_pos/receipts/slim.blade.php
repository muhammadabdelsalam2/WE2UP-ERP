    ...
    <body>
      <div style="margin-right:2%">
           	@if(!empty($receipt_details->logo))
        		<img width="" class="logo" src="{{$receipt_details->logo}}" alt="Logo">
        	@endif
      </div>
      <hr>
        <div class="ticket">
...
@if(!empty($receipt_details->contact))
			...
			<div class="border-top textbox-info">
				<p class="f-"></p>
				<center>
				<p class="f-center" style="margin-right:33%">
					<strong>{!! $receipt_details->invoice_no_prefix !!}</strong>  {{$receipt_details->invoice_no}}
				</p>
				</center>
			</div>
            ...
            <table style="margin-top: 10px !important" class="border-bottom width-100 table-f-12 mb-10">
                <thead class="border-bottom-dotted">
                    <tr>
                        ...
                        <th class="price text-right">الخصم</th>
                ...
                <tbody>
                	...
                	@forelse($receipt_details->lines as $line)
	                    <tr>
	                        ...
	                        <td class="quantity text-right"><?php echo number_format($line['quantity'], 0, '.', ',') ; ?>{{$line['units']}}</td>
	                        @if(empty($receipt_details->hide_price))
	                        
	                        <?php
	                        	$price_before=str_replace( ',', '', $line['unit_price_before_discount']  );
	                        	$total_line=str_replace( ',', '', $line['line_total']  );
	                        	$discount_money=((float)$price_before * (float)$line['quantity']) - ( (float)$total_line);
	                            $discount_total += $discount_money;
	                            $line_total=str_replace( ',', '', $line['line_total'] );
	                        ?>
	                        <td class="unit_price text-right">{{(int)$price_before}}</td>
	                        <td class="price text-right">{{$discount_total}}</td>
	                        <td class="price text-right"><?php echo (int)$line_total ?></td>
	                        @endif
	                    </tr>
		 ...
		 @if(!empty($receipt_details->total_quantity_label))
            ...
            <div class="flex-box">
                <p class="left text-left">
                	<strong style="font-size:16px">{!! $receipt_details->subtotal_label !!}</strong>
                </p>
                <p class="width-50 text-right">
                	<strong style="font-size:16px">{{$receipt_details->subtotal}}</strong>
                </p>
            </div>
			...
			<div class="flex-box">
				<p class="width-50 text-left">
						<strong style="font-size:16px">	{!! $receipt_details->total_label !!}</strong>
				</p>
				<p class="width-50 text-right">
						<strong style="font-size:16px">	{{$receipt_details->total}}</strong>
				</p>
			</div>
            ...
            <!-- Total Paid-->
				@if(!empty($receipt_details->total_paid))
					<div class="flex-box">
						<p class="width-50 text-left">
								<strong style="font-size:16px">	{!! $receipt_details->total_paid_label !!}</strong>
						</p>
				<?php if($discount_total>0){
				$total_b_dis=str_replace( ',', '', $receipt_details->total_paid  );
				$total_b_dis=str_replace( 'EGP', '',$total_b_dis);
                $total_b_dis+=$discount_total;
                $percent_dis=((float)$discount_total / (float)$total_b_dis) *100;
				    	                        	//$price_before=str_replace( ',', '', $line['unit_price_before_discount']  );

				        }
						?>
						<p class="width-50 text-right">
						<strong style="font-size:16px">	{{$receipt_details->total_paid}}</strong>
						</p>
					</div>
				@endif
					@if($discount_total>0)
					<div class="flex-box">
						<p class="width-50 text-left" >
					<strong style="font-size:16px">	وفرت : </strong>			 
						</p>
						<p class="width-50 text-right">
							<strong style="font-size:16px">	EGP	{{$discount_total}} 
						<br>
						({{(int)$percent_dis}}%)</strong>
						</p>
					</div>
				@endif
