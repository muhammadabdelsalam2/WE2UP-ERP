...
{!! Form::open(['url' => action('ProductController@update' , [$product->id] ), 'method' => 'PUT', 'id' => 'product_add_form',
    ...
    @component('components.widget', ['class' => 'box-primary'])
        <div class="row">
         ...
         <div class="col-sm-4">
          <div class="form-group">
            {!! Form::label('name', __(' اقصي عدد داخل الفاتورة  ') . ':') !!}
              {!! Form::text('max_in_invoice',  $product->max_in_invoice, ['class' => 'form-control',
              'placeholder' => __('EX: 5 ')]); !!}
          </div>
        </div>
        ...
        <div class="col-sm-4">
          <div class="form-group">
            {!! Form::label('name', __('  اقصي نسبة خصم     ') . ':') !!}
              {!! Form::text('max_discount',   $product->max_discount, ['class' => 'form-control',
              'placeholder' => __('EX: 5 ')]); !!} %
          </div>
        </div>
							...
							<table class="table table-condensed table-bordered table-th-green text-center table-striped">
								<thead>
									<tr>
										...
										@foreach($price_groups2 as $price_group)
								...
								<tbody>
									@foreach($product->variations as $variation)
										...
										<td><span class="display_currency" data-currency_symbol="true">{{$variation->sell_price_inc_tax}}</span></td>
											@foreach($price_groups2 as $price_group)
												<td>
													{!! Form::text('group_prices[' . $price_group->id . '][' . $variation->id . ']', !empty($variation_prices[$variation->id][$price_group->id]) ? @num_format($variation_prices[$variation->id][$price_group->id]) : 0, ['class' => 'form-control input_number input-sm pg_'.$price_group->id],['id'=>$price_group->id] ); !!}
												<input type="text" id="perc_{{$price_group->id}}" class="form-control" placeholder="نسبة%">
												</td>
												<script>
												
                                                        $('#perc_{{$price_group->id}}').keyup(function (){
                                                            debugger;
                                                           var dpp=$('#single_dpp_inc_tax').val(); 
                                                           var perc=$('#perc_{{$price_group->id}}').val(); 
                                                           var c=perc /100 * dpp;
                                                           c=parseFloat(c) + parseFloat(dpp);
                                                           c=parseFloat(c);
                                                           $(".pg_{{$price_group->id}}").val(c.toFixed(2));
                                                        });
                                                    </script>
                                            @endforeach
										</tr>
