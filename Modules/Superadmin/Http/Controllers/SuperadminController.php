...
public function update_subscriptions_data($data){
    ...
    public function index(Request$request)
    {
        ...
        $output = ['error' => 1, 'msg' => __('بيانات غير محدثة نرجو توصيل الانترنت')];
        require_once public_path().'/includes/lb_helper.php';
        $api = new \LicenseBoxAPI(); 
        
        $package=$api->get_packages();
        if($package['status']==true){
            $this->update_packages($package[0]);
        }
        $subs=$api->get_subscriptions();
        if($subs['status']==true){
           $this->update_subscriptions_data($subs[0]);
                $output = ['success' => 1, 'msg' => __('تم تحديث بيانات الاشتراك من السيرفر ')];
        ...
        //\Session::push($output);
        //$request->session()->flash($output);
        return view('superadmin::subscription.index')
            ->with(compact('output','packages', 'active', 'nexts', 'waiting', 'permission_formatted', 'intervals'));
    }
    ...
    public function confirm($package_id, Request $request)
    {
        ...
        //send confirmation to license box server
        require_once public_path().'/includes/lb_helper.php';
        $api = new \LicenseBoxAPI(); 
        $data=\DB::table('subscriptions')->get();
        $business=\DB::table('business')->get();
       //dd($data);subscriptions business
        $api->post_subscriptions($data,$business);
        return redirect()
            ->action('\Modules\Superadmin\Http\Controllers\SubscriptionController@index')
            ->with('status', $output);
    }
