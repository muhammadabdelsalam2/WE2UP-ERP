    ...
    public function getLedgerDetails($contact_id, $start, $end)
    {
        ...
        foreach ($transactions as $transaction) {
            if($transaction->type=='purchase_return'){
                $total_purchase_return += $transaction->final_total;
            }
...
$i++;
            ...
            $note = $payment->note;
            if (!empty($ref_no)&&$payment->transaction_type !=='purchase') {
                $note .='<small>' . __('الدفع مقابل فاتورة مبيعات') . ': ' . $ref_no . '</small>';
            }
            $transaction_type=$transaction_types['payment'];
            if ($payment->is_advance == 1) {
                $delete_url=url("/payments/deletepayment/{$payment->id}");
                $note .='<small>' . __('lang_v1.advance_payment') . '</small>'.'<a href="'.$delete_url.'"><i class="fa fa-trash"></i></a>';
                $transaction_type='دفع';
                $payment->transaction_type='paid';
            }
            if($payment->transaction_type =='purchase'){
                  $transaction_type='دفع';
                  $note .='<small>' . __('الدفع مقابل فاتورة مشتريات') . ': ' . $ref_no . '</small>';
            }
            
            if($payment->contact_type=='supplier'){
                $transaction_type="استلام مستحق مرتجع مشتريات";
            }
            ...
            $ledger[] = [
                ...
                'debit' => in_array($payment->transaction_type, ['purchase', 'purchase_return','paid']) || ($payment->is_advance == 1 && $payment->contact_type=='customer') ? $payment->amount : '',
                'credit' =>  in_array($payment->transaction_type, array('sell','opening_balance','sell_return')) || $payment->contact_type=='supplier' ? $payment->amount : '',
                'others' =>  $note
            ];
        }
//sd
    ...
    public function payContact($request, $format_data = true)
    {
       ...
       if(isset($request->contact_type) ){
           $inputs['contact_type']=$request->contact_type;
       }
