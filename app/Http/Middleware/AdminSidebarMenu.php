                ...
                $menu->dropdown(
                    ...
                    if (auth()->user()->can('purchase_return.create')) {
                            $sub->url(
                                ...
                                __('مرتجع مشتريات '),
                                ['icon' => 'fa fas fa-undo', 'active' => request()->segment(1) == 'reports']
                            );
                        }
                    },
            ...
            if (auth()->user()->can('sell.view') || auth()->user()->can('sell.create') || auth()->user()->can('direct_sell.access') ||  auth()->user()->can('view_own_sell_only') || auth()->user()->can('access_sell_return')) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if (auth()->user()->can('importsales.create')) {
                            $sub->url(
                        ...
                        if (auth()->user()->can('access_sell_return')) {
                            $sub->url(
                                action('SellController@for_return'),
                                __('مرتجع المبيعات برقم الفاتورة'),
                                ['icon' => 'fa fas fa-file-import', 'active' => request()->segment(1) == 'sells']
                            );
                        }
                        if (auth()->user()->can('access_sell_return')) {
                            $sub->url(
                                action('ReportController@getproductSellReturnReport'),
                                __('مرتجع المبيعات'),
                                ['icon' => 'fa fas fa-file-import', 'active' => request()->segment(1) == 'sells']
                            );
                        }
            ...
            if (in_array('stock_transfers', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create'))) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        //ReportController@compare_locations
                         if (auth()->user()->can('stcok_compares')) {
                            $sub->url(
                                action('ReportController@compare_locations'),
                                'مقارنة المخازن',
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'stock-transfers' && request()->segment(2) == 'stock-compares']
                            );
            ...
            if (in_array('stock_transfers', $enabled_modules) && (auth()->user()->can('stocktacking.view') || auth()->user()->can('stocktacking.create'))) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        $sub->url(
                                action('StocktackingController@index'),
                                __('جرد المخازن'),
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if (auth()->user()->can('less_trending_product_report.view')) {
                            $sub->url(
                                action('ReportController@getLessTrendingProducts'),
                                'المنتجات الراكدة',
                                ['icon' => 'fa fas fa-chart-line', 'active' => request()->segment(2) == 'trending-products']
                            );
                        }
                        ...
                        //
                         $sub->url(
                                action('ReportController@sell_purchase_lines_report'),
                                'حركة صنف',
                                ['icon' => 'fa fas fa-user-secret', 'active' => request()->segment(2) == 'sell-purchase-lines-report']
                            );
                        }
                        //
                    },
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if (auth()->user()->can('download_update') ) {
                            $sub->url(
                                action('LisenceBox\LisenceBoxController@download_update'),
                                __('تحديث النظام'),
                                ['icon' => 'fa fas fa-bolt', 'active' => request()->segment(1) == 'lisenceBox']
                            );
                        }
